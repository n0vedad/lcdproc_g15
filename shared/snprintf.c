...
#if defined(NEED_ASPRINTF) || defined(NEED_ASNPRINTF) || defined(NEED_VASPRINTF) ||                \
    defined(NEED_VASNPRINTF)
	...
	while (*p) {
		if (*p != '%') {
			...
			while (*p == '0' || *p == '-' || *p == '+' || *p == ' ' || *p == '#' ||
				...
				case ' ':
					force_sign = 1;
					/* If both the ' ' and '+' flags appear, the ' ' flag should
					 * be ignored */
			...
			if (*p == '.') {
				...
				if (*p == '*') {
					...
					if (j >= 0)
						...
						precision = 0;
						/* NOTE:
						 *   Solaris 2.6 man page claims that in this case
						 * the precision should be set to 0.  Digital
						 * Unix 4.0, HPUX 10 and BSD man page claim that
						 * this case should be treated as unspecified
			...
			if (*p == 'h' || *p == 'l') {
				...
				if (length_modifier == 'l' &&
				    *p == 'l') { /* double el = long long */
#ifdef SNPRINTF_LONGLONG_SUPPORT
					length_modifier = '2'; /* double letter el encoded as '2' */
#else
					length_modifier =
					    'l'; /* treat it as a single 'l' (letter el) */
#endif
					p++;
				}
			}
			...
			case 's':
				...
				case 'c': {
					const int j = va_arg(ap, int);
					uchar_arg =
					    (unsigned char)j; /* standard demands unsigned char */
			...
			case 'p': {
...
#endif
				if (fmt_spec == 'p') {
					/* HPUX 10: An l, h, ll or L before any other conversion
					 * character (other than d, i, u, o, x, or X) is ignored.
					 * Digital Unix:
					 *   not specified, but seems to behave as HPUX does.
					 * Solaris: If an h, l, or L appears before any other
					 * conversion specifier (other than d, i, u, o, x, or X),
					 * the behavior is undefined. (Actually %hp converts only
					 * 16-bits of address and %llp treats address as 64-bit data
					 * which is incompatible with (void *) argument on a 32-bit
					 * system).
					 */
					...
					case '\0':
					case 'h':
						/* It is non-portable to specify char or short as
						 * the second argument to va_arg, because arguments
				 ...
				 * specified, the '0' flag should be ignored. This is so with
				 * Solaris 2.6, Digital UNIX 4.0, HPUX 10, Linux, FreeBSD, NetBSD;
				 * but not with Perl.
				 */
				...
				if (fmt_spec == 'd') {
					 ...
					 *          a nonzero result is prefixed by 0x. */
						 ...
						 /* Actually it uses 0x prefix even for a zero
						    value. */
						 && arg_sign != 0
#endif
					) {
				...
				if (precision == 0 && arg_sign == 0
				    ...
				    && fmt_spec != 'p'
				/* HPUX 10 man page claims: With conversion character p the result
				 * of converting a zero value with a precision of zero is a null
				 * string. Actually HP returns all zeroes, and Linux returns
				 * "(nil)". */
#endif
				) {
					/* converted to null string */
					/* When zero value is formatted with an explicit precision
					   0, the resulting formatted string is empty (d, i, u, o,
					   x, X, p).   */
				} else {
					static int sprintf_return_value_is_ansi_compliant =
					    -1; /* unknown */
					char f[5];
					int f_l = 0, sprintf_l = 0;
					f[f_l++] =
					...
					f[f_l++] = fmt_spec;
					f[f_l++] = '\0';
					if (sprintf_return_value_is_ansi_compliant <
					    0) { /* not yet known */
						/* let's do a little run-time experiment (only once)
						 * to see if the native sprintf returns a string
						 * length as required by ANSI, or has some other
						 * ideas like the old SunOS which returns buffer
						 * address */
						sprintf_return_value_is_ansi_compliant =
						    (sprintf(tmp + str_arg_l, "%d", 19) == 2);
					}
					if (fmt_spec == 'p')
						sprintf_l = sprintf(tmp + str_arg_l, f, ptr_arg);
					else if (fmt_spec == 'd') { /* signed */
						switch (length_modifier) {
						case '\0':
						case 'h':
							sprintf_l =
							    sprintf(tmp + str_arg_l, f, int_arg);
							break;
						case 'l':
							sprintf_l =
							    sprintf(tmp + str_arg_l, f, long_arg);
							break;
#ifdef SNPRINTF_LONGLONG_SUPPORT
						case '2':
							sprintf_l = sprintf(
						...
						case 'h':
							sprintf_l =
							    sprintf(tmp + str_arg_l, f, uint_arg);
							break;
						case 'l':
							sprintf_l =
							    sprintf(tmp + str_arg_l, f, ulong_arg);
							break;
#ifdef SNPRINTF_LONGLONG_SUPPORT
						case '2':
							sprintf_l = sprintf(
					...
					if (!sprintf_return_value_is_ansi_compliant) { /* broken
											  sprintf?
											*/
						tmp[sizeof(tmp) - 1] = '\0';
						sprintf_l = strlen(tmp + str_arg_l);
					}
					assert(sprintf_l >=
					       0); /* should not happen; problem in sprintf? */
					assert(sprintf_l + str_arg_l <
					       sizeof(tmp)); /*better late then never*/
					str_arg_l += sprintf_l;
                   ...
                   && tmp[zero_padding_insertion_ind] == '0')
						...
						if (!precision_specified ||
							...
							/* precision is increased to force the first
							   character to be zero, except if a zero
							   value is formatted with an explicit
							   precision of zero */
							precision = num_of_digits + 1;
							precision_specified = 1;
						}
			...
			case 'n': {
				...
				if (ptr != NULL) {
					 ...
					 * snprintf return value - see comment at the end of this
					 * procedure */
					switch (length_modifier) {
					case '\0':
						*(int *const)ptr = str_l;
						break;
					case 'h':
						*(short int *const)ptr = str_l;
						break;
					case 'l':
						*(long int *const)ptr = str_l;
						break;
#ifdef SNPRINTF_LONGLONG_SUPPORT
					case '2':
						*(long long int *const)ptr = str_l;
						break;
#endif
					}
				}
