...
static int process_config(ConfigSection **current_section,
			  char (*get_next_char)(),
 ...
 */
const char *
config_get_string(const char *sectionname, const char *keyname, int skip, const char *default_value)
{
...
#endif
{
	...
	while (state != ST_END) {
		...
		case ST_ASSIGNMENT:
			...
			default:
				report(RPT_WARNING,
				       "Assignment expected on line %d of %s: %s",
				       line_nr,
				       source_descr,
				       keyname);
				error = 1;
