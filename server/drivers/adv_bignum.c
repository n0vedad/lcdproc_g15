...
/** \file
	...
	\li Needed functions:
		...
		\li \c drvthis->chr()
		\li \c drvthis->height()
	\li Custom characters (if available) at char offset+0, offset+1, ...,
...
\code
	...
	serialVFD_num(Driver *drvthis, int x, int num)
	{
		...
		if (p->ccmode != CCMODE_BIGNUM) { // Are the customcharacters set up correctly? If
not: do_init = 1;	// Lib_adv_bignum has to set the customcharacters. p->ccmode =
CCMODE_BIGNUM; // Switch customcharactermode to bignum.
		}
...
static void adv_bignum_num_2_6(Driver *drvthis, int x, int num, int height, int offset, int do_init)
{
	...
	if (do_init) {
		int i;
		static unsigned char bignum[6][8] = {
		    /* stored customcharacter */
		    [0] =
			{
			    b__XXXXX,
			    b__XXXXX,
