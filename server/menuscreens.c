...
void menuscreen_switch_item(MenuItem *new_menuitem)
{
	...
	debug(RPT_DEBUG,
	      "%s(item=[%s]) from active_menuitem=[%s]",
	      __FUNCTION__,
...
void menuscreen_create_menu(void)
{
	 ...
	 * server
	 */
	checkbox = menuitem_create_checkbox(
	    "heartbeat", heartbeat_handler, "Heartbeat", NULL, true, heartbeat);
	menu_add_item(options_menu, checkbox);

	checkbox = menuitem_create_checkbox(
	    "backlight", backlight_handler, "Backlight", NULL, true, backlight);
	menu_add_item(options_menu, checkbox);

	slider = menuitem_create_slider("titlespeed",
					titlespeed_handler,
					"TitleSpeed",
					NULL,
					"0",
	...
	for (driver = drivers_getfirst(); driver; driver = drivers_getnext()) {
		...
		if (contrast_avail || brightness_avail) {
			...
			if (driver_menu == NULL) {
				report(RPT_ERR,
				       "%s: Cannot create menu for driver %s",
				       __FUNCTION__,
				       driver->name);
				continue;
			}
			...
			if (contrast_avail) {
				...
				/* menu's client is NULL since we're in the server */
				slider = menuitem_create_slider("contrast",
								contrast_handler,
								"Contrast",
								NULL,
								"min",
			...
			if (brightness_avail) {
				...
				slider = menuitem_create_slider("onbrightness",
								brightness_handler,
								"On Brightness",
								NULL,
								"min",
				...
				slider = menuitem_create_slider("offbrightness",
								brightness_handler,
								"Off Brightness",
								NULL,
								"min",
...
void menuscreen_create_testmenu(void)
{
	...
	test_item = menuitem_create_ring(
	    ...
	    "Ring",
	    NULL,
	    "ABC\tDEF\t01234567890\tOr a very long string that will not fit on any display",
	    1);
	menu_add_item(test_menu, test_item);
	...
	menu_add_item(test_menu, test_item);
	test_item = menuitem_create_slider(
	    "", NULL, "Slider,step=5", NULL, "mintext", "maxtext", -20, 20, 5, 0);
	menu_add_item(test_menu, test_item);
	...
	test_item = menuitem_create_alpha(
	    "", NULL, "Alpha", NULL, 0, 3, 12, true, true, true, ".-+@", "LCDproc-v0.5");
	menu_add_item(test_menu, test_item);
	test_item = menuitem_create_alpha(
	    "", NULL, "Alpha, caps only", NULL, 0, 3, 12, true, false, false, "-", "LCDPROC");
	menu_add_item(test_menu, test_item);
		...
		if (driver != NULL) {
			if (strcmp(item->id, "onbrightness") == 0) {
				driver->set_brightness(
				    driver, BACKLIGHT_ON, item->data.slider.value);
			} else if (strcmp(item->id, "offbrightness") == 0) {
				driver->set_brightness(
				    driver, BACKLIGHT_OFF, item->data.slider.value);
			}
		}
	}
	return 0;
...
void menuscreen_add_screen(Screen *s)
{
	...
	mi = menuitem_create_ring("",
				  NULL,
				  "Priority",
				  s->client,
				  "Hidden\tBackground\tForeground\tAlert\tInput",
				  s->priority);
	menu_add_item(m, mi);
}
