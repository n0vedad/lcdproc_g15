// SPDX-License-Identifier: GPL-2.0+

/**
 * \file clients/lcdexec/menu.c
 * \brief Menu parsing and building functions for the lcdexec client
 * \author Joris Robijn, Peter Marschall
 * \date 2002-2007
 *
 * \features
 * - Recursive menu hierarchy parsing from configuration files
 * - Multiple input parameter types (sliders, rings, numeric, text, IP, checkbox)
 * - Menu structure serialization for LCDd server communication
 * - Memory management for complex menu structures
 * - Debug output and menu introspection
 * - ID-based menu entry lookup for server callbacks
 * - Automatic menu generation and management
 *
 * \usage
 * - Configure menu structures in lcdexec configuration files
 * - Use menu_read() to parse and build menu hierarchies
 * - Use menu_sock_send() to register menus with LCDd server
 * - Use menu_find_by_id() for server callback handling
 * - Use menu_free() for memory cleanup
 *
 * \details This file implements the menu system for lcdexec, providing
 * functionality to parse menu configurations from config files and build
 * hierarchical menu structures. It supports various input parameter types
 * and handles communication with the LCDd server for menu display.
 */

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <strings.h>
#include <unistd.h>

#include "menu.h"

#include "shared/configfile.h"
#include "shared/report.h"
#include "shared/sockets.h"

/** \brief Boolean value name lookup table */
static char *boolValueName[] = {"false", "true"};
/** \brief Tri-state with gray value name lookup table */
static char *triGrayValueName[] = {"off", "on", "gray"};

// Recursively read and parse menu hierarchy from configuration
MenuEntry *menu_read(MenuEntry *parent, const char *name)
{
	static int id = 0;

	// Allocate and initialize menu entry if configuration section exists
	if ((name != NULL) && (config_has_section(name))) {
		MenuEntry *me = calloc(1, sizeof(MenuEntry));

		if (me == NULL)
			return NULL;

		me->id = id++;
		me->name = strdup(name);
		if (me->name == NULL) {
			menu_free(me);
			return NULL;
		}

		me->displayname = strdup(config_get_string(name, "DisplayName", 0, name));
		if (me->displayname == NULL) {
			menu_free(me);
			return NULL;
		}

		me->parent = parent;
		me->next = NULL;
		me->children = NULL;
		me->numChildren = 0;

		// Check if this is a submenu (has Entry configurations)
		if (config_get_string(name, "Entry", 0, NULL) != NULL) {
			MenuEntry **addr = &me->children;
			const char *entryname;

			me->type = MT_MENU;

			while ((entryname = config_get_string(name, "Entry", me->numChildren,
							      NULL)) != NULL) {
				MenuEntry *entry = menu_read(me, entryname);

				if (entry == NULL) {
					menu_free(me);
					return NULL;
				}

				me->numChildren++;
				*addr = entry;
				addr = &entry->next;
			}

			// Check if this is an executable command (has Exec configuration)
		} else if (config_get_string(name, "Exec", 0, NULL) != NULL) {
			MenuEntry **addr = &me->children;
			const char *entryname;

			me->type = MT_EXEC;

			me->data.exec.command = strdup(config_get_string(name, "Exec", 0, ""));
			if (me->data.exec.command == NULL) {
				menu_free(me);
				return NULL;
			}
			me->data.exec.feedback = config_get_bool(name, "Feedback", 0, 0);

			while ((entryname = config_get_string(name, "Parameter", me->numChildren,
							      NULL)) != NULL) {
				MenuEntry *entry = menu_read(me, entryname);

				if (entry == NULL) {
					menu_free(me);
					return NULL;
				}

				me->numChildren++;
				*addr = entry;
				addr = &entry->next;
			}

			// Automagically add an "Apply" action for commands with parameters
			if ((me->numChildren > 0) && (addr != NULL))
				*addr = menu_read(me, NULL);

			// Parse command parameter types from configuration
		} else if (config_get_string(name, "Type", 0, NULL) != NULL) {
			const char *type;

			type = config_get_string(name, "Type", 0, "");

			// Slider parameter: range input with min/max values
			if (strcasecmp(type, "slider") == 0) {
				char buf[35];

				me->type = MT_ARG_SLIDER;

				me->data.slider.value = config_get_int(name, "Value", 0, 0);
				me->data.slider.minval = config_get_int(name, "MinValue", 0, 0);
				me->data.slider.maxval = config_get_int(name, "MaxValue", 0, 1000);

				snprintf(buf, sizeof(buf), "%d", me->data.slider.minval);
				me->data.slider.mintext =
				    strdup(config_get_string(name, "MinText", 0, buf));
				snprintf(buf, sizeof(buf), "%d", me->data.slider.maxval);
				me->data.slider.maxtext =
				    strdup(config_get_string(name, "MaxText", 0, buf));

				me->data.slider.stepsize = config_get_int(name, "StepSize", 0, 1);

				// Ring parameter: selection from predefined string list
			} else if (strcasecmp(type, "ring") == 0) {
				const char *tmp;
				int numStrings = 0;
				int i = 0;

				me->type = MT_ARG_RING;

				me->data.ring.value = config_get_int(name, "Value", 0, 0);

				numStrings = config_has_key(name, "String");
				me->data.ring.strings =
				    (char **)calloc(sizeof(char *), numStrings + 1);
				me->data.ring.strings[numStrings] = NULL;

				while ((tmp = config_get_string(name, "String", i, NULL)) != NULL) {
					me->data.ring.strings[i] = strdup(tmp);
					i++;
				}

				me->data.ring.strings[i] = NULL;

				// Numeric parameter: integer input with min/max constraints
			} else if (strcasecmp(type, "numeric") == 0) {
				me->type = MT_ARG_NUMERIC;

				me->data.numeric.value = config_get_int(name, "Value", 0, 0);
				me->data.numeric.minval = config_get_int(name, "MinValue", 0, 0);
				me->data.numeric.maxval = config_get_int(name, "MaxValue", 0, 1000);

				// Alpha parameter: text input with character restrictions
			} else if (strcasecmp(type, "alpha") == 0) {
				me->type = MT_ARG_ALPHA;

				me->data.alpha.value =
				    strdup(config_get_string(name, "Value", 0, ""));

				me->data.alpha.minlen = config_get_int(name, "MinLength", 0, 0);
				me->data.alpha.maxlen = config_get_int(name, "MaxLength", 0, 100);

				me->data.alpha.allowed = strdup(config_get_string(
				    name, "AllowedChars", 0, "ABCDEFGHIJKLMNOPQRSTUVWXYZ"));

				// IP parameter: IPv4/IPv6 address input
			} else if (strcasecmp(type, "ip") == 0) {
				me->type = MT_ARG_IP;

				me->data.ip.value = strdup(config_get_string(name, "Value", 0, ""));
				me->data.ip.v6 = config_get_bool(name, "Value", 0, 0);

				// Checkbox parameter: boolean/tristate selection
			} else if (strcasecmp(type, "checkbox") == 0) {
				const char *tmp;

				me->type = MT_ARG_CHECKBOX;

				me->data.checkbox.allow_gray =
				    config_get_bool(name, "AllowGray", 0, 0);

				me->data.checkbox.value =
				    (me->data.checkbox.allow_gray)
					? config_get_tristate(name, "Value", 0, "gray", 0)
					: config_get_bool(name, "Value", 0, 0);

				tmp = config_get_string(name, "OffText", 0, NULL);
				me->data.checkbox.map[0] = (tmp != NULL) ? strdup(tmp) : NULL;

				tmp = config_get_string(name, "OnText", 0, NULL);
				me->data.checkbox.map[1] = (tmp != NULL) ? strdup(tmp) : NULL;

				tmp = config_get_string(name, "GrayText", 0, NULL);
				me->data.checkbox.map[2] = (tmp != NULL) ? strdup(tmp) : NULL;

			} else {
				report(RPT_DEBUG, "illegal parameter type");
				menu_free(me);
				return NULL;
			}

		} else {
			report(RPT_DEBUG, "unknown menu entry type");
			menu_free(me);
			return NULL;
		}

		return me;

		// Magic stuff: Auto-generate Apply action for EXEC entries
	} else {
		if ((name == NULL) && (parent != NULL) && (parent->type == MT_EXEC)) {
			MenuEntry *me = calloc(1, sizeof(MenuEntry));

			if (me == NULL)
				return NULL;

			me->id = id++;

			me->name = malloc(strlen(parent->name) + 10);
			if (me->name == NULL) {
				menu_free(me);
				return NULL;
			}

			snprintf(me->name, strlen(parent->name) + 10, "Apply_%s", parent->name);

			me->displayname = strdup("Apply!");
			if (me->displayname == NULL) {
				menu_free(me);
				return NULL;
			}

			me->parent = parent;
			me->next = NULL;
			me->children = NULL;
			me->numChildren = 0;

			me->type = MT_ARG_ACTION_AUTO;

			return me;
		}
	}

	return NULL;
}
/**
 * \brief Set menu item to quit on selection
 * \param me Menu entry to configure
 * \param sock Socket file descriptor for server communication
 * \retval 0 Success
 * \retval -1 Send failed
 *
 * \details Sends menu_set_item command to configure the menu entry to
 * trigger _quit_ action when selected, but only if it's the last entry.
 */
static int menu_set_quit(MenuEntry *me, int sock)
{
	if (me->next != NULL)
		return 0;

	return sock_printf(sock, "menu_set_item {} {%d} -next _quit_\n", me->id);
}

// Create LCDproc commands for menu hierarchy and send to server
int menu_sock_send(MenuEntry *me, MenuEntry *parent, int sock)
{
	if ((me != NULL) && (sock > 0)) {
		char parent_id[12];

		if ((parent != NULL) && (parent->id != 0))
			snprintf(parent_id, sizeof(parent_id), "%d", parent->id);
		else
			parent_id[0] = 0;

		// Process menu entry based on its type using LCDd protocol
		switch (me->type) {
			MenuEntry *entry;

		// Handle menu entries - create submenus in LCDd
		case MT_MENU:
			if ((parent != NULL) && (me->id != 0)) {
				if (sock_printf(sock, "menu_add_item \"%s\" \"%d\" menu \"%s\"\n",
						parent_id, me->id, me->displayname) < 0)
					return -1;
			}

			for (entry = me->children; entry != NULL; entry = entry->next) {
				if (menu_sock_send(entry, me, sock) < 0)
					return -1;
			}
			break;

		// Handle executable commands
		case MT_EXEC:
			if (me->children == NULL) {
				if (sock_printf(sock, "menu_add_item \"%s\" \"%d\" action \"%s\"\n",
						parent_id, me->id, me->displayname) < 0)
					return -1;

				if (sock_printf(sock, "menu_set_item {} {%d} -menu_result quit\n",
						me->id) < 0)
					return -1;
			} else {
				if ((parent != NULL) && (me->id != 0)) {
					if (sock_printf(sock,
							"menu_add_item \"%s\" \"%d\" menu \"%s\"\n",
							parent_id, me->id, me->displayname) < 0)
						return -1;
				}

				for (entry = me->children; entry != NULL; entry = entry->next) {
					if (menu_sock_send(entry, me, sock) < 0)
						return -1;
				}
			}
			break;

		// Handle slider parameter input
		case MT_ARG_SLIDER:
			if (sock_printf(sock,
					"menu_add_item \"%s\" \"%d\" slider -text \"%s\""
					" -value %d -minvalue %d -maxvalue %d"
					" -mintext \"%s\" -maxtext \"%s\" -stepsize %d\n",
					parent_id, me->id, me->displayname, me->data.slider.value,
					me->data.slider.minval, me->data.slider.maxval,
					me->data.slider.mintext, me->data.slider.maxtext,
					me->data.slider.stepsize) < 0)
				return -1;

			if (menu_set_quit(me, sock) < 0)
				return -1;

			break;

		// Handle ring parameter selection
		case MT_ARG_RING: {
			int i;
			char *tmp = strdup("");
			if (tmp == NULL)
				break;

			// Concatenate all ring option strings with TAB separators for LCDd
			for (i = 0; me->data.ring.strings[i] != NULL; i++) {
				size_t current_len = strlen(tmp);
				size_t new_len =
				    current_len + 1 + strlen(me->data.ring.strings[i]) + 1;
				char *new_tmp = realloc(tmp, new_len);
				if (new_tmp != NULL) {
					tmp = new_tmp;
					if (current_len > 0)
						strncat(tmp, "\t", new_len - strlen(tmp) - 1);
					strncat(tmp, me->data.ring.strings[i],
						new_len - strlen(tmp) - 1);
				} else {
					free(tmp);
					tmp = NULL;
					break;
				}
			}

			if (tmp != NULL) {
				if (sock_printf(sock,
						"menu_add_item \"%s\" \"%d\" ring -text \"%s\""
						" -value %d -strings \"%s\"\n",
						parent_id, me->id, me->displayname,
						me->data.ring.value, tmp) < 0) {
					free(tmp);
					return -1;
				}
				free(tmp);
			} else {
				if (sock_printf(sock,
						"menu_add_item \"%s\" \"%d\" ring -text \"%s\""
						" -value %d -strings \"\"\n",
						parent_id, me->id, me->displayname,
						me->data.ring.value) < 0)
					return -1;
			}

			if (menu_set_quit(me, sock) < 0)
				return -1;

			break;
		}

		// Handle numeric parameter input with constraints
		case MT_ARG_NUMERIC:
			if (sock_printf(sock,
					"menu_add_item \"%s\" \"%d\" numeric -text \"%s\""
					" -value %d -minvalue %d -maxvalue %d\n",
					parent_id, me->id, me->displayname, me->data.numeric.value,
					me->data.numeric.minval, me->data.numeric.maxval) < 0)
				return -1;

			if (menu_set_quit(me, sock) < 0)
				return -1;

			break;

		// Handle alpha text parameter input with character restrictions
		case MT_ARG_ALPHA:
			if (sock_printf(sock,
					"menu_add_item \"%s\" \"%d\" alpha -text \"%s\""
					" -value \"%s\" -minlength %d -maxlength %d"
					" -allow_caps false -allow_noncaps false"
					" -allow_numbers false -allowed_extra \"%s\"\n",
					parent_id, me->id, me->displayname, me->data.alpha.value,
					me->data.alpha.minlen, me->data.alpha.maxlen,
					me->data.alpha.allowed) < 0)
				return -1;

			if (menu_set_quit(me, sock) < 0)
				return -1;

			break;

		// Handle IP address parameter input for IPv4/IPv6
		case MT_ARG_IP:
			if (sock_printf(sock,
					"menu_add_item \"%s\" \"%d\" ip -text \"%s\""
					" -value \"%s\" -v6 %s\n",
					parent_id, me->id, me->displayname, me->data.ip.value,
					boolValueName[me->data.ip.v6]) < 0)
				return -1;

			if (menu_set_quit(me, sock) < 0)
				return -1;

			break;

		// Handle checkbox parameter input for boolean/tristate values
		case MT_ARG_CHECKBOX:
			if (sock_printf(sock,
					"menu_add_item \"%s\" \"%d\" checkbox -text \"%s\""
					" -value %s -allow_gray %s\n",
					parent_id, me->id, me->displayname,
					triGrayValueName[me->data.checkbox.value],
					boolValueName[me->data.checkbox.allow_gray]) < 0)
				return -1;

			if (menu_set_quit(me, sock) < 0)
				return -1;

			break;

		// Handle auto-generated action entries for command execution
		case MT_ARG_ACTION_AUTO:
			if (sock_printf(sock, "menu_add_item \"%s\" \"%d\" action \"%s\"\n",
					parent_id, me->id, me->displayname) < 0)
				return -1;

			if (sock_printf(sock, "menu_set_item {} {%d} -menu_result quit\n", me->id) <
			    0)
				return -1;
			break;

		// Handle unknown menu entry types
		default:
			report(RPT_ERR, "Unknown menu entry type: %d", me->type);
			return -1;
		}
		return 0;
	}
	return -1;
}

// Find menu entry by its unique ID
MenuEntry *menu_find_by_id(MenuEntry *me, int id)
{
	if (me != NULL) {
		if (me->id == id)
			return me;

		if ((me->type == MT_MENU) || (me->type == MT_EXEC)) {
			MenuEntry *entry;

			for (entry = me->children; entry != NULL; entry = entry->next) {
				MenuEntry *result = menu_find_by_id(entry, id);

				if (result != NULL)
					return result;
			}
		}
	}
	return NULL;
}

// Get the command string from an executable menu entry
const char *menu_command(MenuEntry *me)
{
	if ((me != NULL) && (me->type == MT_EXEC))
		return me->data.exec.command;

	return NULL;
}

// Free all memory associated with a menu structure
void menu_free(MenuEntry *me)
{
	if (me != NULL) {
		MenuEntry *entry;
		int i;

		switch (me->type) {

		// Free executable command data and fall through to menu cleanup
		case MT_EXEC:
			if (me->data.exec.command != NULL)
				free(me->data.exec.command);
			me->data.exec.command = NULL;

		// Free menu hierarchy and all children recursively
		case MT_MENU:
			for (entry = me->children; entry != NULL;) {
				MenuEntry *old = entry;

				entry = entry->next;
				old->next = NULL;
				menu_free(old);
			}
			me->children = NULL;
			break;

		// Free slider parameter text strings
		case MT_ARG_SLIDER:
			if (me->data.slider.mintext != NULL)
				free(me->data.slider.mintext);
			me->data.slider.mintext = NULL;
			if (me->data.slider.maxtext != NULL)
				free(me->data.slider.maxtext);
			me->data.slider.maxtext = NULL;
			break;

		// Free ring parameter string array
		case MT_ARG_RING:
			if (me->data.ring.strings != NULL) {
				int i;

				for (i = 0; me->data.ring.strings[i] != NULL; i++)
					free(me->data.ring.strings[i]);

				free((void *)me->data.ring.strings);
				me->data.ring.strings = NULL;
			}
			break;

		// Free alpha parameter value and allowed character strings
		case MT_ARG_ALPHA:
			if (me->data.alpha.value != NULL)
				free(me->data.alpha.value);
			me->data.alpha.value = NULL;
			if (me->data.alpha.allowed != NULL)
				free(me->data.alpha.allowed);
			me->data.alpha.allowed = NULL;
			break;

		// Free IP parameter value string
		case MT_ARG_IP:
			if (me->data.ip.value != NULL)
				free(me->data.ip.value);
			me->data.ip.value = NULL;
			break;

		// Free checkbox parameter text mapping strings
		case MT_ARG_CHECKBOX:
			for (i = 0;
			     i < sizeof(me->data.checkbox.map) / sizeof(me->data.checkbox.map[0]);
			     i++) {
				if (me->data.checkbox.map[i] != NULL) {
					free(me->data.checkbox.map[i]);
					me->data.checkbox.map[i] = NULL;
				}
			}

		// No additional cleanup needed for other types
		default:
			break;
		}

		if (me->name != NULL)
			free(me->name);
		me->name = NULL;

		if (me->displayname != NULL)
			free(me->displayname);
		me->displayname = NULL;

		me->type = MT_UNKNOWN;

		free(me);
	}
}

// Debug function to dump menu structure to stdout
#if defined(DEBUG)
void menu_dump(MenuEntry *me)
{
	if (me != NULL) {
		if (me->type & MT_AUTOMATIC)
			return;

		report(RPT_DEBUG, "# menu ID: %d", me->id);
		report(RPT_DEBUG, "[%s]", me->name);
		if (me->displayname != NULL)
			report(RPT_DEBUG, "DisplayName=\"%s\"", me->displayname);

		switch (me->type) {
			MenuEntry *entry;

		// Debug dump menu structure and recursively dump children
		case MT_MENU:
			for (entry = me->children; entry != NULL; entry = entry->next)
				report(RPT_DEBUG, "Entry=%s", entry->name);
			report(RPT_DEBUG, "");

			for (entry = me->children; entry != NULL; entry = entry->next)
				menu_dump(entry);
			break;

		// Debug dump executable command and its parameters
		case MT_EXEC:
			report(RPT_DEBUG, "Exec=\"%s\"", me->data.exec.command);
			report(RPT_DEBUG, "Feedback=%s", boolValueName[me->data.exec.feedback]);

			for (entry = me->children; entry != NULL; entry = entry->next)
				report(RPT_DEBUG, "Parameter=%s", entry->name);
			report(RPT_DEBUG, "");

			for (entry = me->children; entry != NULL; entry = entry->next)
				menu_dump(entry);
			break;

		// Debug dump slider parameter configuration
		case MT_ARG_SLIDER:
			report(RPT_DEBUG, "Type=slider");
			report(RPT_DEBUG, "Value=%d", me->data.slider.value);
			report(RPT_DEBUG, "MinValue=%d", me->data.slider.minval);
			report(RPT_DEBUG, "MaxValue=%d", me->data.slider.maxval);
			report(RPT_DEBUG, "Stepsize=%d", me->data.slider.stepsize);
			report(RPT_DEBUG, "MinText=%s", me->data.slider.mintext);
			report(RPT_DEBUG, "MaxText=%s", me->data.slider.maxtext);
			report(RPT_DEBUG, "");
			break;

		// Debug dump ring parameter configuration and strings
		case MT_ARG_RING:
			report(RPT_DEBUG, "Type=ring");
			report(RPT_DEBUG, "Value: %d", me->data.ring.value);
			{
				int i;

				for (i = 0; me->data.ring.strings[i] != NULL; i++)
					report(RPT_DEBUG, "String=\"%s\"",
					       me->data.ring.strings[i]);
			}
			report(RPT_DEBUG, "");
			break;

		// Debug dump numeric parameter configuration
		case MT_ARG_NUMERIC:
			report(RPT_DEBUG, "Type=numeric");
			report(RPT_DEBUG, "Value=%d", me->data.numeric.value);
			report(RPT_DEBUG, "MinValue=%d", me->data.numeric.minval);
			report(RPT_DEBUG, "MaxValue=%d", me->data.numeric.maxval);
			report(RPT_DEBUG, "");
			break;

		// Debug dump alpha parameter configuration
		case MT_ARG_ALPHA:
			report(RPT_DEBUG, "Type=alpha");
			report(RPT_DEBUG, "Value=\"%s\"", me->data.alpha.value);
			report(RPT_DEBUG, "AllowedChars=\"%s\"", me->data.alpha.allowed);
			report(RPT_DEBUG, "");
			break;

		// Debug dump IP parameter configuration
		case MT_ARG_IP:
			report(RPT_DEBUG, "Type=ip");
			report(RPT_DEBUG, "Value=\"%s\"", me->data.ip.value);
			report(RPT_DEBUG, "V6=%s", boolValueName[me->data.ip.v6]);
			report(RPT_DEBUG, "");
			break;

		// Debug dump checkbox parameter configuration
		case MT_ARG_CHECKBOX:
			report(RPT_DEBUG, "Type=checkbox");
			report(RPT_DEBUG, "Value=%s", triGrayValueName[me->data.checkbox.value]);
			report(RPT_DEBUG, "AllowGray=%s",
			       boolValueName[me->data.checkbox.allow_gray]);

			if (me->data.checkbox.map[0] != NULL)
				report(RPT_DEBUG, "OffText=%s", me->data.checkbox.map[0]);
			if (me->data.checkbox.map[1] != NULL)
				report(RPT_DEBUG, "OnText=%s", me->data.checkbox.map[1]);
			if (me->data.checkbox.map[2] != NULL)
				report(RPT_DEBUG, "GrayText=%s", me->data.checkbox.map[2]);
			report(RPT_DEBUG, "");
			break;

		// Handle unknown menu entry types in debug output
		default:
			report(RPT_DEBUG, "ERROR: unknown menu entry type: %d", me->type);
			break;
		}
	}
}
#endif
