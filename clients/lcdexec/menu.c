...
MenuEntry *menu_read(MenuEntry *parent, const char *name)
{
	...
	if ((name != NULL) && (config_has_section(name))) {
		...
		if (config_get_string(name, "Entry", 0, NULL) != NULL) {
			...
			// read menu entries
			while ((entryname = config_get_string(
				    name, "Entry", me->numChildren, NULL)) != NULL) {
			...
			while ((entryname = config_get_string(
				    name, "Parameter", me->numChildren, NULL)) != NULL) {
				MenuEntry *entry = menu_read(me, entryname);
	...
	if ((me != NULL) && (sock > 0)) {
		...
		case MT_EXEC:
			if (me->children == NULL) {
				...
				if (sock_printf(sock,
						"menu_set_item {} {%d} -menu_result quit\n",
						me->id) < 0)
					return -1;
			} else {
				if ((parent != NULL) && (me->id != 0)) {
	...
	if (me != NULL) {
		...
		case MT_ARG_RING:
			...
			report(RPT_DEBUG, "Value: %d", me->data.ring.value);
			{
				...
				for (i = 0; me->data.ring.strings[i] != NULL; i++)
					report(
					    RPT_DEBUG, "String=\"%s\"", me->data.ring.strings[i]);
			}
			report(RPT_DEBUG, "");
			break;
		case MT_ARG_NUMERIC:
		...
		case MT_ARG_CHECKBOX:
			...
			report(
			    RPT_DEBUG, "AllowGray=%s", boolValueName[me->data.checkbox.allow_gray]);
